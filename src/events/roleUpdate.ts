...
export const event = {
  ...
  async execute(client: Client, oldRole: Role, newRole: Role) {
    ...
    if (oldRole.unicodeEmoji !== newRole.unicodeEmoji)
      changes.push({ name: "Emoji",       value: `${oldRole.unicodeEmoji ?? "none"} → ${newRole.unicodeEmoji ?? "none"}`,                  inline: true  });
    ...
    try {
      ...
      if (entry && entry.targetId === newRole.id && (Date.now() - entry.createdTimestamp) < 5000)
    ...
    const embed = new EmbedBuilder()
      .setColor(0x000000)
      .setAuthor({ name: "Role Updated", iconURL: guildIcon })
      .setDescription(
        `Role <@&${newRole.id}> was updated${executor ? ` by <@${executor}>` : ""}`
      )
      .addFields(...changes)
      .setTimestamp()
