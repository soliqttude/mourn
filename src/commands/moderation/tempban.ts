...
export const command: HybridCommand = {
  ...
  async execute(ctx) {
    ...
    try {
      await ctx.guild.members.ban(target.id, { reason: `[TEMPBAN ${durStr}] ${ctx.user.tag}: ${reason}` });
      await db.insert(tempBans).values({ guildId: ctx.guild.id, userId: target.id, moderatorId: ctx.user.id, reason, unbanAt });
      const caseId = await logCase(ctx.guild.id, target.id, ctx.user.id, "tempban", reason, durStr);
