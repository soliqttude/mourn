...
export const command: HybridCommand = {
  ...
  async execute(ctx) {
    ...
    try {
      ...
      return ctx.reply({ embeds: [new EmbedBuilder().setColor(0xff69b4).setDescription(`**${ctx.user.username}** high fives **${target.username}**! ✋`).setImage(data.url).setFooter({ text: `${config.embedFooter}` }).setTimestamp()] });
    } catch {
