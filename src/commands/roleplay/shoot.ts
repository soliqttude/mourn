...
export const command: HybridCommand = {
  ...
  async execute(ctx) {
    ...
    try {
      const res = await fetch("https://api.waifu.pics/sfw/shoot");
      const data = await res.json() as { url: string };
      return ctx.reply({ embeds: [new EmbedBuilder().setColor(0xff69b4).setDescription(`**${ctx.user.username}** shoots **${target.username}**! 🔫`).setImage(data.url).setFooter({ text: `${config.embedFooter}` }).setTimestamp()] });
    } catch {
