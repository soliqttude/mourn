...
export const command: HybridCommand = {
  name: "vmreset",
  aliases: ["vreset"],
  description: "Reset all permission overrides on your voice channel.",
  usage: "vmreset",
  examples: ["vmreset"],
  ...
  async execute(ctx) {
    ...
    for (const [id, overwrite] of vc.permissionOverwrites.cache) {
      if (id !== ctx.guild.id) await overwrite.delete().catch(() => {});
    }
    await vc.permissionOverwrites.edit(ctx.guild.roles.everyone, { Connect: null, ViewChannel: null });
    return ctx.reply({ embeds: [successEmbed("voice channel permissions reset.")] });
  },
};