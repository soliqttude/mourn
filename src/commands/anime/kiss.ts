...
export const command: HybridCommand = {
  ...
  async execute(ctx) {
    try {
      ...
      return ctx.reply({ embeds: [new EmbedBuilder().setColor(0xff69b4).setTitle("💋 Kiss").setImage(data.url).setFooter({ text: `Requested by ${ctx.user.username} • ${config.embedFooter}` }).setTimestamp()] });
    } catch {
